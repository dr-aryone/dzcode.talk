...
function values()
{
    ...
    yield from ['string','array',100]; //the from key will yield at every value so it will stop at every value
    yield from array("hello" => 100,'amine'=>"hammou"); //it will yield the key from every value 100 HAMMOU
}
$yld = values();
...
foreach ($yld as $key) {
    ...
    //json pretty print cant print callable objects u just have to call the function of yield
}
function calling()
{
    yield function () {
...
echo '<br>'. $gen->getReturn(); //the last return after a yield the order here is important
